...
def prepare_ct_data(ct_file_objects, start_dt, end_dt,
    ...
    try:
        ...
        _ct_app_spec = _ilu.spec_from_file_location('ct_app_module', os.path.join(_HERE, 'app.py'))
        if 'ct_app_module' not in sys.modules:
            _ct_app_mod = _ilu.module_from_spec(_ct_app_spec)
            sys.modules['ct_app_module'] = _ct_app_mod
            _ct_app_spec.loader.exec_module(_ct_app_mod)
        else:
            _ct_app_mod = sys.modules['ct_app_module']
        _compute_failure_series = _ct_app_mod._compute_failure_series
...
def build_main_report(df_ct_clean, df_anomaly, df_wb_total, df_wb_rejected,
    ...
    if 'ct_app_module' not in sys.modules:
        ...
        _ct_app_mod = sys.modules['ct_app_module']
    build_full_report = _ct_app_mod.build_full_report
    drop_unnamed      = _ct_app_mod.drop_unnamed
    drop_computed     = _ct_app_mod.drop_computed
    CT_COMPUTED       = _ct_app_mod.CT_COMPUTED
...
def build_summary_report(df_wb_total, demo_template_path=None,
    ...
    import importlib.util as _ilu
    _fd_spec = _ilu.spec_from_file_location('ct_fill_demo', os.path.join(_HERE, 'fill_demo.py'))
    if 'ct_fill_demo' not in sys.modules:
        _fd_mod = _ilu.module_from_spec(_fd_spec)
        sys.modules['ct_fill_demo'] = _fd_mod
        _fd_spec.loader.exec_module(_fd_mod)
    else:
        _fd_mod = sys.modules['ct_fill_demo']
    fill_demo_report = _fd_mod.fill_demo_report
