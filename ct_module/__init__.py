...
# ── In-memory store (per server process, reset on redeploy) ───────────────────
_ct_store = {
...
def ct_build_main():
    ...
    global _ct_store
    try:
        if _ct_store['df_ct'] is None:
            return jsonify({'error': 'Run /ct/process first'}), 400
        eng  = _get_ct_engine()
        xlsx = eng.build_main_report(
            _ct_store['df_ct'], _ct_store['df_anomaly'],
            _ct_store['df_wb_total'], _ct_store['df_wb_rejected'],
            _ct_store['failure_list'], _ct_store['start_dt'], _ct_store['end_dt'])
        _ct_store['main_xlsx'] = xlsx
...
def ct_build_ppt():
    ...
    try:
        if _ct_store['df_ct'] is None:
            return jsonify({'error': 'Run /ct/process first'}), 400
        if _ct_store['summary_xlsx'] is None:
            return jsonify({'error': 'Build summary first'}), 400
        eng = _get_ct_engine()
        ppt = eng.build_ppt_report(
            _ct_store['df_ct'], _ct_store['df_wb_total'],
            reason=_ct_store['reason'], exceedances=_ct_store['exceedances'],
            applicable_hours=_ct_store['applicable_hours'],
            summary_xlsx_bytes=_ct_store['summary_xlsx'],
            ppt_template_path=os.path.join(_CT_DIR, 'demo.pptx'))
        _ct_store['ppt_bytes'] = ppt
